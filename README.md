...
| `-a:<codepage\|name>` | Decode text DFMs that have no UTF-8 BOM using the given ANSI encoding instead of assuming UTF-8 without BOM. Accepts a Windows code page number (`-a:1250`), an IANA encoding name (`-a:iso-8859-2`, `-a:windows-1250`, `-a:koi8-r`) or the `cpNNNN` form (`-a:cp1250`). Use this for legacy DFMs that fail with *"No mapping for the Unicode character exists in the target multi-byte code page"*. Files that already start with a UTF-8 BOM are not affected by this option. |
...
# Same, naming the encoding instead of the code page number
DFMStabilizerTool -a:iso-8859-2 legacy\*.dfm
```
